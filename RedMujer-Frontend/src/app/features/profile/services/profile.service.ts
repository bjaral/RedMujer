...
export class ProfileService {
  ...
  obtenerPersona(personaId: number): Observable<any> {
    return this.http.get<any>(`${this.url}/Personas/${personaId}`);
  }

  // Obtener ubicación
  obtenerUbicacion(ubicacionId: number): Observable<any> {
    return this.http.get<any>(`${this.url}/Ubicaciones/${ubicacionId}`);
  }
  ...
  return this.obtenerUsuario(userId).pipe(
    switchMap(usuario => {
      ...
      return this.http.get<any[]>(`${this.url}/Personas`).pipe(
        map(personas => {
  ...
  actualizarUsuario(userId: number, datos: any): Observable<any> {
    return this.http.put<any>(`${this.url}/Usuarios/${userId}`, datos);
  }

  // Actualizar persona
  actualizarPersona(personaId: number, datos: any): Observable<any> {
    return this.http.put<any>(`${this.url}/Personas/${personaId}`, datos);
  }

  // Actualizar ubicación
  actualizarUbicacion(ubicacionId: number, datos: any): Observable<any> {
    return this.http.put<any>(`${this.url}/Ubicaciones/${ubicacionId}`, datos);
  }
  ...
  cambiarContrasena(userId: number, contrasenaActual: string, contrasenaNueva: string): Observable<any> {
    return this.http.put<any>(`${this.url}/Usuarios/${userId}/cambiar-contrasena`, {
      contrasenaActual,
      contrasenaNueva
    });
  }
